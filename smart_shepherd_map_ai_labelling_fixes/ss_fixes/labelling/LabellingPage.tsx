...
export default function LabellingPage() {
  ...
  // ── Build flagged animals from CRITICAL/WARNING alerts ──────────────────
  // FIX: remplace les 3 mocks hardcodés par les vraies alertes du store IoT
  const flaggedAnimals = useMemo((): FlaggedAnimal[] => {
    const criticalAlerts = alerts.filter(a =>
      (a.severity === 'CRITICAL' || a.severity === 'WARNING') && !a.read
    );

    // Dédupliquer par animal
    const seen = new Set<string>();
    const result: FlaggedAnimal[] = [];

    for (const alert of criticalAlerts) {
      const animalId = alert.collar_id || alert.animal_id || String(alert.id);
      if (seen.has(animalId)) continue;
      seen.add(animalId);

      const liveData = positions[animalId] || positions[alert.animal_name || ''];

      result.push({
        id: animalId,
        name: alert.animal_name || `Animal #${animalId}`,
        anomalyDate: new Date(alert.timestamp).toISOString().split('T')[0],
        severity: alert.severity === 'CRITICAL' ? 'Critical' : 'High',
        type: (alert.type || 'Unknown anomaly').replace(/_/g, ' '),
    ...
    // Fallback si aucune alerte → utiliser les animaux en état CRITICAL du store
    if (result.length === 0) {
      Object.values(positions)
        .filter(a => a.status === 'CRITICAL' || a.status === 'OUT_OF_ZONE')
        .slice(0, 10)
        .forEach(a => {
          const id = a.collar_id || a.sheepId || a._id;
          if (!id || seen.has(id)) return;
          seen.add(id);
          result.push({
            id,
            name: a.name || `Animal #${id}`,
            anomalyDate: new Date().toISOString().split('T')[0],
            severity: a.status === 'CRITICAL' ? 'Critical' : 'High',
            type: a.status === 'CRITICAL' ? 'Anomalie critique' : 'Sortie de zone',
  ...
  // ── Auto-select first flagged animal ─────────────────────────────────────
  useEffect(() => {
    if (!selectedAnimal && flaggedAnimals.length > 0) {
      setSelectedAnimal(flaggedAnimals[0]);
    }
  }, [flaggedAnimals, selectedAnimal]);
  ...
  // FIX: chartData utilise la télémétrie réelle si disponible, sinon génère des données stables
  const chartData = useMemo(() => {
    const labels = Array.from({ length: 48 }, (_, i) => `${i}h`);

    if (telemetryData && telemetryData.length > 0) {
      // Données réelles
      const temps = telemetryData.map((d: any) => d.temperature ?? 38.5);
      const bpms = telemetryData.map((d: any) => d.heartRate ?? d.bpm ?? 80);
      return { labels: labels.slice(0, Math.max(temps.length, bpms.length)), temps, bpms };
    }

    // Données de démonstration stables (pas Math.random() à chaque render)
    const liveTemp = selectedAnimal?.temperature ?? 38.5;
    const liveBpm = selectedAnimal?.heartRate ?? 80;
    const seed = selectedAnimal?.id?.charCodeAt(0) ?? 42;
    const pseudoRand = (i: number, amp: number) => Math.sin(i * 0.5 + seed) * amp;

    return {
      labels,
      temps: Array.from({ length: 48 }, (_, i) => parseFloat((liveTemp + pseudoRand(i, 0.8)).toFixed(1))),
      bpms: Array.from({ length: 48 }, (_, i) => Math.round(liveBpm + pseudoRand(i, 8))),
    };
  }, [telemetryData, selectedAnimal]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`✓ Diagnostic soumis — ${selectedAnimal.name} (${OUTCOME_LABELS[outcome]})`);
            ...
            <div className="overflow-y-auto max-h-[420px]">
              {flaggedAnimals.map(a => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 border-b border-gray-50 dark:border-gray-800/50 last:border-b-0 ${
                    selectedAnimal?.id === a.id ? 'bg-primary/5 border-r-2 border-primary' : ''
                  }`}
                >
                  ...
                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded flex-shrink-0 ${
                    a.severity === 'Critical' ? 'bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400'
              ...
              <form onSubmit={handleSubmit} className="bg-white dark:bg-card-dark rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-5 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white px-3 py-2 resize-none focus:outline-none focus:border-primary placeholder-gray-400"
                  />
                </div>
                ...
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <div
                    ...
                    className={`w-10 h-5 rounded-full transition-colors flex-shrink-0 ${isConfirmed ? 'bg-primary' : 'bg-gray-200 dark:bg-gray-700'}`}
                  >
                    <div className={`w-4 h-4 rounded-full bg-white shadow m-0.5 transition-transform ${isConfirmed ? 'translate-x-5' : 'translate-x-0'}`} />
                  </div>
                  <span className="text-sm text-gray-700 dark:text-gray-200">Confirmé par un vétérinaire (validation officielle)</span>
                </label>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full px-5 py-3 rounded-xl bg-primary text-white text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {diagnoseMutation.isPending
                    ? <><Loader2 className="w-4 h-4 animate-spin" /> Envoi…</>
                    : <><CheckCircle2 className="w-4 h-4" /> Soumettre le diagnostic</>
                  }
                </button>
              </form>
            )}
