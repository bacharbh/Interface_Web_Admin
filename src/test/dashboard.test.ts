...
vi.mock('../contexts/MqttContext', () => ({ useMqtt: () => ({ isConnected: true, isSimulation: false, toggleSimulation: vi.fn() }) }));
...
beforeEach(() => { useIoTStore.setState({ devices: { A1: makeAnimal({ collar_id: 'A1', battery: 12, temperature: 78 }) }, history: {}, alerts: [makeAlert({ id: 'a1' })], aiAlerts: [], needsCharging: [], isConnected: false, isSimulation: true, isOfflineData: false }); });

it('renders KPI cards and stays quiet about performance in test mode', async () => {
    ...
    expect(warnSpy.mock.calls.filter(([message]) => String(message).includes('Dashboard loaded') || String(message).includes('Dashboard rendered'))).toHaveLength(0);
});