...
const AnimalMarker = React.memo(({ animal, history = [], isSelected, onSelect }: AnimalMarkerProps) => {
  ...
  const cacheKey = `${animal.status}_${isSelected}_${anomaly.label}`;
  ...
  if (!customIcon) {
    ...
    const animationClass = (animal.status === 'CRITICAL' || isCriticalML)
      ? 'animate-pulse scale-110'
      : 'animate-float';

    const glowClass = (animal.status === 'CRITICAL' || isCriticalML)
    ...
    customIcon = L.divIcon({
      html: `
        <div class="relative group ${animationClass}">
          <div class="w-9 h-9 rounded-full border-2 border-white dark:border-gray-800 shadow-2xl flex items-center justify-center transition-all duration-500 ${glowClass}" style="background-color: ${statusColor}">
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-white drop-shadow-sm">
              <path d="M16 16c-1.1 0-2-.9-2-2 0-.2.1-.4.1-.6-.8-.3-1.6-.3-2.3 0 .1.2.1.4.1.6 0 1.1-.9 2-2 2s-2-.9-2-2c0-.2.1-.4.1-.6C7.2 13 6.6 12 6.6 10.9c0-2.3 2.1-4.2 4.6-4.2h1.6c2.5 0 4.6 1.9 4.6 4.2 0 1.1-.6 2.1-1.4 2.6.1.2.1.4.1.6 0 1.1-.9 2-2 2z"/>
              <path d="M8 16v4"/><path d="M16 16v4"/>
            </svg>
          </div>
          ${(animal.status === 'CRITICAL' || isCriticalML) ? '<div class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full border-2 border-white dark:border-gray-800 animate-ping shadow-lg"></div>' : ''}
          ${isSelected ? '<div class="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-primary rounded-full animate-bounce"></div>' : ''}
        </div>
      `,
