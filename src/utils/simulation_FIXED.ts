...
// ✅ CORRECTION : Centrage par défaut sur l'utilisateur ou fallback France
let currentCenter = { lat: 48.8566, lng: 2.3522 }; // Paris par défaut
let userLocation: { lat: number; lng: number } | null = null;

// ✅ Fonction pour définir la position utilisateur
export const setUserLocation = (lat: number, lng: number) => {
  userLocation = { lat, lng };
  currentCenter = { lat, lng };
};

// ✅ Fonction pour obtenir la position actuelle (utilisateur ou défaut)
export const getCurrentCenter = () => {
  return userLocation || currentCenter;
};
